...
寄存器 | 含义 | 寄存器字段
 - | - | -
DPL | 段描述符表中的特权级，规定了这个短的特权保护级别，只有不低于此级别，才可以访问段描述符指向的短。 | 不存在寄存器中，在内存的段描述符中
CPL | 当前的特权级 | CS中的两位
RPL | 请求特权级，会结合CPL和RPL的最低特权判断能否对某段访问 | 选择子中的两位

访问条件 DPL > MAX(CPL, RPL)TT
> 但不明白为什么那个blog里面说RPL可以有很多位
