...
int buddy2_alloc(struct buddy2* self, int size) {
    ...
    if (size <= 0)
        ...
        size = fixsize(size);   // 找到最小的2的幂
    
    if (self->longest[index] < size) // 如果可用内存小于请求的块，则返回失败
        return -1;
    
    for(node_size = self->size; node_size != size; node_size /= 2 ) { // 寻找第一个可以分配的块。
        if (self->longest[LEFT_LEAF(index)] >= size) // 如果左孩子可以分配的比要求的大，则从左孩子里面分配。
            index = LEFT_LEAF(index);
        else
            index = RIGHT_LEAF(index);
    }
    
    self->longest[index] = 0; // 找到了index分配出去，并且把可用内存置0
    offset = (index + 1) * node_size - self->size; // 求内存的起始地址
    
    while (index) {
        index = PARENT(index); // 更新父亲
        self->longest[index] =
...
void buddy2_free(struct buddy2* self, int offset) {
    ...
    node_size = 1;
    index = offset + self->size - 1; // 找到归还块对应的节点编号
    
    // 确定这个索引块所对应的大小
    for (; self->longest[index] ; index = PARENT(index)) { // 当查找的节点可用的时候
        node_size *= 2;
        if (index == 0)
            return;
    }
...
int buddy2_print_index(struct buddy2* self, int index) {
    ...
    if (node_size == self->longest[index]) {
        printf("| %d ----", offset);
        return self->longest[index];
    } else if (self->longest[index] == 0){
        printf("| %d XXXX", offset);
        return 0;
    } else {
        return buddy2_print_index(self, LEFT_LEAF(index)) + buddy2_print_index(self, RIGHT_LEAF(index));
    }
    return node_size;
}

int buddy2_print(struct buddy2* self) {
    int res = buddy2_print_index(self, 0);
    printf("\n");
    return res;
